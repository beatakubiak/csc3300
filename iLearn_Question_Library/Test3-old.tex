...
Department the number of course sections he failed (received a grade 'F' for).\par
\begin{verbatim}
    create role teaching_assistant;
    create role lecturer;
    create role dean;
    grant select on takes to teaching_assistant;
    grant update on takes to teaching_assistant;
    grant select on student to lecturer;
    grant select on instructor to dean;
    grant teaching_assistant to lecturer;
    grant lecturer to dean;
    grant dean to Grey;
    grant dean to Amit;
    grant update on instructor to Amit;
    grant lecturer to Brown;
    grant update on student to Brown;
    grant teaching_assistant to Green;
\end{verbatim}
(Select all that apply)\par\vspace{0.5em}
\begin{checkboxes}
    \CorrectChoice Amit
...
which user(s) have the authority to enter student's \textbf{grade}, for some course section, and also update his \textbf{tot\_cred}.\par
\begin{verbatim}
...
\end{verbatim}
\textbf{Hint: } To test it in MYSQL you need to invoke the following commands.\par
\begin{verbatim}
